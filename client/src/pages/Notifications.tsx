...
import {
  ...
  Star, UserPlus, ShieldCheck, ExternalLink, Clock
} from "lucide-react";
...
function getActionLink(notification: any): string | null {
  ...
  if (notification.type === "ESCROW_UPDATE" && data.escrowId) {
    return `/checkout/${data.escrowId}`;
  }
  if (notification.type === "MESSAGE") return "/dashboard";
  if (notification.type === "REVIEW") return "/dashboard";
  if (notification.type === "FOLLOW_REQUEST") return "/dashboard";
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(notifications as any[]).map((n: any) => {
              const cfg = TYPE_CONFIG[n.type] || TYPE_CONFIG.ESCROW_UPDATE;
              const Icon = cfg.icon;
